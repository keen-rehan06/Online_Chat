...
export const userRegisterChecks = async (req, res, next) => {
    try {
        ...
        if (!emailRegex.test(email)) return res.status(401).send({
            message: 'Invalid email format',
            success: false
        });
        if (password.length < 6) return res.status(401).send({ message: "Password must be at least 6 characters" });
        if (password.length > 12) return res.status(401).send({ message: "Too Long Password only 12 character Password allowed.", success: false });
        next();
    } catch (error) {
...
export const userLoginChecks = async (req, res, next) => {
    try {
        ...
        if (typeof email !== "string" || typeof password !== "string") return res.status(401).send({ message: "All filed must be string", success: false })
        const emailRegex = /^[^\s@]+@[^\s@]+\.[^\s@]+$/;
        if (!emailRegex.test(email)) return res.status(401).send({ message: "Invalid Email Format", success: false });
        const user = await userModel.findOne({ email });
...
export const isVerifiedtoken = async (req, res, next) => {
    try {
        ...
        if (!token) return res.status(401).send({ message: "Token Not found!", error })
        let decoded;
        try {
